...
self.addEventListener('fetch', function (event) {
  ...
  //            network response.
    return cache.match(event.request).then(function (cachedResponse) {
        ...
        // Otherwise, fetch the resource from the network instead
        // Per instructions: replaced fetch(event.request.url) with fetch(event.request)
        return fetch(event.request).then(function (networkResponse) {
          // Clone the response because responses can only be read once.
          // One copy goes to the cache, the other goes back to the browser page.
